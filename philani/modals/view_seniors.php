...
if(isset($_POST['id']))
{
	...
	//DOUBLES
	$doubles = mysqli_query($con, "SELECT * FROM players WHERE age > '18' AND AC_number != '{$ac_number}'");
}
ob_start();
?>
...
<div class="modal fade" id="viewSenoirModal" tabindex="-1" role="dialog" aria-labelledby="viewSenoirModalLabel">
                ...
                <div class="row col-md-4" id="assigneInvesrigater">
                    <div id="assigneInvesrigater_div" style="display:block;">
                    	<form action="controller/update_matches_seniors.php" method="post">
                            ...
                            <!--================================Hidden inputs==========================-->
                        	<table class="table table-bordered" width="100%" style="margin-bottom:10%; margin-top:10%;">                    	
                                <thead>                                    
                                    <tr>            	
                                        <th>Name</th>
                                        <th>Surname</th>
  ...
  <script>
  		function closeModal()
		{
			jQuery('#viewSenoirModal').modal('hide');
			setTimeout 
			(
				function()
				{
					jQuery('#viewSenoirModal').remove();
					jQuery('#viewSenoirModal').remove();
				},500
			);
		}
