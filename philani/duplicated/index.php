<?php 
	...
	<?php }
	$check_j = mysqli_query($con, "SELECT * FROM players WHERE age < 19 AND menager = '{$_SESSION['email']}'");
	$check_s = mysqli_query($con, "SELECT * FROM players WHERE age > 18 AND menager = '{$_SESSION['email']}'");	
...
<legend><hr></legend>
  <!-- Trigger the modal with a button -->
  	<legend>
  	    <?php if(mysqli_num_rows($check_j) > 0): ?>
  	    <a onClick='juniorsTable("1")' class="btn btn-sm" style="background-color:#f44336;color:#fff;">Juniors</a>
  	    <?php 
  	        endif; 
  	        if(mysqli_num_rows($check_s) > 0):
  	    ?>
  	    <a onClick='seniorsTable("2")' class="btn btn-sm" style="background-color:rgba(60,109,111,1.00); color:#fff;">Seniors</a>
  	    <?php endif; ?>
    ...
    <?php if(!isset($_GET['seniors'])){ ?>
  	<table class="table table-responsive table-bordered" cellspacing="0" style="margin-bottom:10%; width:100%">                    	
        <thead>
        	<tr><th colspan="19"><h2 style="background-color:#f44336; color:#fff; padding:1%;"><?php echo $_SESSION['club_name']; ?> (Juniors)</h2></th></tr>
            <tr>            	
                <th>#</th>
                <th>AC Number</th>
                <th>Name</th>
    ...
    <?php }else{ ?>
    <table class="table table-bordered table-responsive" width="100%" cellspacing="0" style="margin-bottom:10%;">                    	
        <thead>
  ...
  <script>
  	function juniorsTable(id)
		{
			var data = {"id": id}
			jQuery.ajax
			(
				{
					url: "modals/tables/juniors_table.php",
					method: "post",
					data: data,
					success: function(data)
					{
						jQuery('body').append(data);
						$('#juniorsTableModal').modal('toggle');
					},
					error: function()
					{
						alert('error');
		...
		function seniorsTable(id)
		{
			var data = {"id": id}
			jQuery.ajax
			(
				{
					url: "modals/tables/seniors_table.php",
					method: "post",
					data: data,
					success: function(data)
					{
						jQuery('body').append(data);
						$('#seniorsTableModal').modal('toggle');
					},
					error: function()
					{
						alert('error');
